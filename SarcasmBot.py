...
async def on_message(message):
    ...
    if is_sarcastic:
        response = await message.channel.send(f"{message.author.mention} Stop Being Sarcastic")
        await response.add_reaction('👍')
        await response.add_reaction('👎')
        user_messages[response.id] = message.content  # Store original message content by response ID
...
async def on_reaction_add(reaction, user):
    ...
    if reaction.emoji == '👍' or reaction.emoji == '👎':
        original_message = user_messages.get(message.id)  # Retrieve original message content
...
async def evaluate_model(ctx):
    feedback_data = pd.read_csv(feedback_data_file)
    if len(feedback_data) < 2:  # Ensure there's enough data to split
    ...
    training_args = TrainingArguments(
        ...
        logging_dir='./logs',            
        logging_steps=10,
        eval_strategy="epoch"  # Use the updated evaluation strategy
    )
    ...
    # Send evaluation results to the user
